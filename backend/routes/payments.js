...
// Client-side confirmation: called by the frontend after stripe.confirmPayment()
// succeeds in environments where Stripe webhooks cannot reach the backend
// (local dev without ngrok / stripe listen). The webhook handler below remains
// the authoritative source of truth in production.
router.post('/confirm', async (req, res, next) => {
  try {
    ...
    if (pi.metadata?.backer_id !== user.id) {
      return res.status(403).json({ error: 'PaymentIntent does not belong to this user' });
    }

    const nextStatus =
      pi.status === 'succeeded' ? 'completed'
      : pi.status === 'requires_payment_method' || pi.status === 'canceled' ? 'failed'
      : null;

    if (!nextStatus) {
      return res.status(202).json({ status: pi.status, message: 'Payment still processing' });
    }

    // Idempotency: only update if still pending (webhook may have already processed it)
    ...
    if (error) {
      return res.status(500).json({ error: `Failed to update donation: ${error.message}` });
    }

    // data is null when webhook already processed it — still return success
    res.json({ donation: data, payment_intent_status: pi.status });
  } catch (err) {
    next(err);
  }
});
  ...
  try {
    switch (event.type) {
      case 'payment_intent.succeeded': {
        ...
        // Idempotency: only transition from pending to avoid double-processing on Stripe retries
