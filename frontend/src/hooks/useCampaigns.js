...
// ─── Update campaign details (creator / admin) ───────────────────────────────
export function useUpdateCampaign() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, campaign, rewardTiers }) => {
      const { data, error } = await supabase
        .from('campaigns')
        .update(campaign)
        .eq('id', id)
      ...
      // Replace reward tiers: delete existing, insert updated set
      if (rewardTiers !== undefined) {
        ...
        if (rewardTiers.length > 0) {
          const tiers = rewardTiers.map((t) => ({ ...t, campaign_id: id }));
          const { error: insError } = await supabase.from('reward_tiers').insert(tiers);
...
export function useUpdateCampaignStatus() {
  ...
  return useMutation({
    mutationFn: async ({ id, status, rejection_reason }) => {
      ...
      if (status === 'active') payload.rejection_reason = null; // clear on approve
      const { data, error } = await supabase
        .from('campaigns')
        .update(payload)
        .eq('id', id)
