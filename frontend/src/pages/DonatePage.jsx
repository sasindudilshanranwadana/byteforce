...
import { ArrowLeft, Heart, Lock, CheckCircle, Lightbulb } from 'lucide-react';
...
function PaymentStep({ amount, tier, onBack, campaignId, paymentIntentId }) {
  ...
  async function handleConfirm(e) {
    ...
    try {
      const { error, paymentIntent } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${window.location.origin}/campaigns/${campaignId}/success?amount=${amount}`,
        },
      ...
      // Client-side confirm fallback for environments without webhook delivery
      // (local dev). Backend retrieves the PI, verifies status, and flips
      // donation -> completed so the raised_amount trigger fires.
      if (paymentIntent?.status === 'succeeded') {
        try {
          ...
          await fetch(`${import.meta.env.VITE_API_URL}/api/payments/confirm`, {
            ...
            body: JSON.stringify({ payment_intent_id: paymentIntentId ?? paymentIntent.id }),
          });
        } catch {
          // Webhook will still reconcile in production; safe to ignore here
        }
      }

      const params = new URLSearchParams({ amount: String(amount) });
      if (tier?.title) params.set('tier', tier.title);
      navigate(`/campaigns/${campaignId}/success?${params}`, { replace: true });
    } catch (err) {
          ...
          <Card>
            <CardContent className="pt-6">
              {campaign.image_url ? (
                ...
                <div className="rounded-xl bg-primary-50 w-full aspect-video flex items-center justify-center mb-4">
